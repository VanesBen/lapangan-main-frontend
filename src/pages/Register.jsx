...
export default function Register() {
  ...
  const [role, setRole] = useState("customer"); // default 'customer' atau 'admin'
  ...
  const handleSubmit = (e) => {
    ...
    axiosClient
      ...
      .catch((err) => {
        console.error("Proses register error:", err);
        ...
        if (response && response.status === 422) {
          ...
          setAlert({
            type: "error",
            message: msg || "Validasi gagal. Cek kembali data kamu!",
          });
        } else {
          setAlert({
            type: "error",
            message: response?.data?.message || "Gagal mendaftar ke server!",
          });
        }
      })
      .finally(() => {
  ...
  return (
    <div className="min-h-screen bg-[#0f1117] flex items-center justify-center p-4 font-sans">
      {/* Card Container */}
      <div className="w-full max-w-[440px] bg-[#252629]/90 backdrop-blur-md border border-gray-800 rounded-[32px] p-8 md:p-10 text-white shadow-2xl my-8">
        
        {/* Header */}
        <div className="text-center mb-6">
          <h1 className="text-3xl font-black tracking-tight mb-2 text-white">Register</h1>
          <p className="text-gray-400 text-sm font-light leading-relaxed">
            Buat akun baru untuk mulai sewa lapangan!
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="relative flex items-center">
            ...
            <select
              ...
              className="w-full bg-[#18191c] border border-gray-800 text-white placeholder-gray-500 rounded-full py-3.5 pl-12 pr-4 text-sm focus:outline-none focus:border-[#ccff00] transition-all appearance-none cursor-pointer"
            >
              <option value="customer" className="bg-[#18191c]">Customer</option>
              <option value="admin" className="bg-[#18191c]">Admin</option>
            </select>
        ...
        <div className="text-center mt-6">
          <Link to="/login" className="text-xs text-gray-400 hover:text-[#ccff00] font-medium transition-colors">
